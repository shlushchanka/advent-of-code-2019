...
def find_path(board):
  ...
  (robot_row, robot_col) = find_robot(board)
  # board[robot_row][robot_col] = '#'   
  isScaffold = lambda r,c: 0 <= r < row_count and 0 <= c < col_count and (board[r][c] == '#' or board[r][c] == '^')
  isScaffoldAtDirection = lambda r, c, d: isScaffold(r + directions[d][0], c + directions[d][1])
  directions = [(-1, 0), (0, 1), (1, 0), (0, -1)]
  left_index = lambda direction_index: (direction_index - 1 + len(directions)) % len(directions)
  right_index = lambda direction_index: (direction_index + 1) % len(directions)
  robot_direction_index = 0
  path = []
  while True:
    if isScaffoldAtDirection(robot_row, robot_col, right_index(robot_direction_index)):
      robot_direction_index = right_index(robot_direction_index)
      (row_delta, col_delta) = directions[robot_direction_index]
      robot_row += row_delta
      robot_col += col_delta
      steps = 1
      while isScaffold(robot_row + row_delta, robot_col + col_delta):
        robot_row += row_delta
        robot_col += col_delta
        steps += 1
      path.append('R' + str(steps))
    elif isScaffoldAtDirection(robot_row, robot_col, left_index(robot_direction_index)):
      robot_direction_index = left_index(robot_direction_index)
      (row_delta, col_delta) = directions[robot_direction_index]
      robot_row += row_delta
      robot_col += col_delta
      steps = 1
...
if __name__ == '__main__':
  ...
  for line in lines:
    for c in line:
      std_in.append(ord(c))
  input[0] = 2    
  computer = IntCodeComputer(input, std_in=std_in)
  computer.run()
  print(computer.std_out)
