...
def parse_wire_string(wire):
  pathes = wire.split(',')
  return [parse_path(p) for p in pathes]

def parse_path(path):
  direction = parse_direction_symbol(path[0])
  distance = parse_distance(path[1:])
  return (direction, distance)
  
def parse_direction_symbol(direction_symbol):
...
def visited_points(origin, wire_string):
  ...
  for w in wire:
    ...
    current_position = iterate(point_to_steps, current_position, direction, distance)
  return point_to_steps
...
if __name__ == '__main__':
  ...
  p = min(intersection, key=distance_function)
  print(distance_function(p))
