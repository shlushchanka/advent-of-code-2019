...
def filled_coordinates(program):
  ...
  std_in = []
  std_out = []
  robot = IntCodeComputer(program, std_in, std_out)
  from collections import defaultdict
  panel_color = defaultdict(lambda: 0)
  panel_color[(row, col)] = 1
  while not robot.is_halted:
    std_in.append(panel_color[(row, col)])
    robot.run()
    panel_color[(row, col)] = std_out.pop(0)
    direction_index_delta = -1 if std_out.pop() == 0 else 1
    direction_index = (direction_index + direction_index_delta) % len(directions)
    (row_delta, col_delta) = directions[direction_index]
    row += row_delta
    col += col_delta
  filled_coordinates = [p for p in panel_color if panel_color[p] == 0]
...
def render_image(filled_coordinates):
  ...
  x, y = [i[0] for i in filled_coordinates], [i[1] for i in filled_coordinates]
  image = np.zeros((max(x) - min(x) + 1, max(y) - min(y) + 1))

  for i in range(len(filled_coordinates)):
    image[-min(x) + x[i], -min(y) + y[i]] = 1
